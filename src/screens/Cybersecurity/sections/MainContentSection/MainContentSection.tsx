...
const companyLogos = [{ src: "/frame-271.svg", alt: "Company logos" }];

const statistics = [
  {
    value: "10+",
    label: "Years of Experience",
    valueClasses: "font-bold tracking-[2.59px]",
    labelClasses:
      "[font-family:'Lato',Helvetica] text-2xl tracking-[0.29px] leading-[39.4px]",
  },
  {
    value: "900+",
...
export const MainContentSection = (): JSX.Element => {
  return (
    <section className="flex flex-col items-center gap-[104px] w-full px-4 py-8 relative">
      <div className="flex flex-col items-center gap-4 w-full max-w-[1232px]">
      ...
      <div className="flex flex-col items-center gap-[104px] w-full max-w-[1232px]">
        <div className="flex flex-col items-center gap-12 w-full">
          <p className="w-full max-w-[1222px] [font-family:'Montserrat',Helvetica] font-normal text-2xl text-center tracking-[0] leading-[39.4px]">
          ...
          <div className="relative w-full max-w-[1232px] rounded-[40px] bg-[linear-gradient(90deg,rgba(84,244,252,1)_0%,rgba(198,240,255,1)_37%,rgba(180,243,255,0.9)_75%,rgba(23,109,180,1)_100%)] p-12">
            <div className="flex flex-wrap items-center justify-center gap-[114px]">
              {statistics.map((stat, index) => (
                <div
                  key={index}
                  className="flex flex-col items-center text-center"
                >
                  <div className="[font-family:'Montserrat',Helvetica] font-normal text-black text-7xl tracking-[3.60px] leading-[118.1px]">
                    <span className={stat.valueClasses}>
                      {stat.value}
                      <br />
                    </span>
                    <span className={stat.labelClasses}>{stat.label}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-col items-center gap-12 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[72px] w-full">
            {services.map((service, index) => (
              <Card
                key={index}
                className="flex flex-col items-center gap-4 px-[72px] py-12 rounded-2xl border-[none] bg-transparent before:content-[''] before:absolute before:inset-0 before:p-px before:rounded-2xl before:[background:linear-gradient(114deg,rgba(85,244,252,1)_0%,rgba(24,110,180,1)_100%)] before:[-webkit-mask:linear-gradient(#fff_0_0)_content-box,linear-gradient(#fff_0_0)] before:[-webkit-mask-composite:xor] before:[mask-composite:exclude] before:z-[1] before:pointer-events-none"
              >
                <CardContent className="flex flex-col items-center gap-6 p-0">
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <h3 className="bg-[linear-gradient(129deg,rgba(85,244,252,1)_0%,rgba(26,111,181,1)_100%)] [-webkit-background-clip:text] bg-clip-text [-webkit-text-fill-color:transparent] [text-fill-color:transparent] [font-family:'Montserrat',Helvetica] font-semibold text-2xl text-center tracking-[0] leading-[33.6px] whitespace-nowrap">
                      {service.title}
                    </h3>

                    <p className="max-w-[449px] [font-family:'Lato',Helvetica] font-light text-white text-lg text-center tracking-[0.36px] leading-[31.7px]">
                      {service.description}
                    </p>
                  </div>
                </CardContent>
